...
-  [x] 可以切換模式 變成Delta V
-  [x] 可以設定電壓差上下限
-  [x] 可以設定欄位 多列(壓差) or 單列(容量)
-  [x] 讀 csv上傳 並讀取對應列來上傳
-  [ ] 加入local紀錄的模式
-  [ ] 把Cloud API模式跟local record模式做成可切換
Index to 列
### 伺服器讀取與呈現
-  [x] 讀取所有的Stage (measurement)
-  [x] 伺服器加入開放式API 不鎖定項目
-  [x] 數據項目 X軸時間 Y軸為開放式格式
-  [x] 每個 Y軸都可以模糊搜尋 並要可以多項模糊搜尋
-  [x] 可以匯出CSV
	- 目前想法 先讓使用者選擇Stage 然後選擇要的Field跟排序查詢列 數據預覽 然後匯出csv或excel
### 分選
