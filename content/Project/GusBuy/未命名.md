...
[ 用戶瀏覽器 / PWA ] 
        │
        ▼ (靜態資源、離線快取請求)
[ Nginx / CDN 邊緣節點 ] ──( 靜態網頁 / 圖片快取 )
        │
        ▼ (動態 API 請求: /api/v1/* )
[ Python FastAPI (中間件 / API Gateway) ] ──( 分散式鎖 / 快取 )── [ Redis Cluster ]
        │
        ├─► [ 第三方金流 API (綠界) ]
        ├─► [ 第三方電子發票 API ]
        │
        ▼ (系統 Master Key 安全存取)
[ BaaS: Appwrite Server (資料儲存 / 會員驗證集群) ]
```
...
```
[客戶點擊結帳] ──► [FastAPI 中間件] ──► [獲取 Redis 分散式鎖]
                                              │
                    ┌─────────────────────────┴────────────────────────┐
             (庫存充足, 進入分流)                                  (庫存不足)
                    │                                                  │
         ┌──────────┴──────────┐                                  [拋出庫存不足異常]
    (總額 < 10萬 且 無專案商品)  (總額 >= 10萬 或 有專案商品)
         │                     │
 [建立 PENDING_PAY 訂單]  [建立 PM_ESTIMATING 訂單]
         │                     │
 [呼叫綠界 API 線上刷卡]     [封鎖線上支付] ──► [通知 PM 團隊]
         │
 [Webhook 異步回調確認]
         │
 ┌───────┴───────┐
...
3.  **後端對稱攤分與雙向鎖死：** 請求送達 FastAPI 中間件後，執行以下原子操作交易：
    ...
    * **明細攤分：** 為了維持「財務一致性」，中間件必須依照比例，將 1 萬 5 的折讓金額依權重攤分至該訂單旗下的所有 Item 的 `price_at_purchase` 欄位中，確保：$$\sum (Order\_Items\_Table.price\_at\_purchase) == Order\_Table.total\_amount$$ 避免日後部分退貨時產生明細與總價對不起來的財務災難。
